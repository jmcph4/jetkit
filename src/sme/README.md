# Solidity Metadata Extractor #

`sme` is the **S**olidity **M**etadata **E**xtractor. It extracts the metadata from a smart contract that is, by default, appended to its *runtime* bytecode by the reference implementation of the Solidity compiler, `solc`.
...
Options:
  -r, --raw                  Interpret input (from either standard input or a file via --bytecode) as literal bytes
  -m, --metadata             Print metadata representation to standard output
  -g, --gateway              Display IPFS digests as URLs to the default IPFS web gateway
...
Metadata {
    digest: Some(
        Ipfs(
            "QmbRQF3HA9eZ8ZLwyUtnQfXjvZ8MTRnKWTVULhqLALUSTu",
        ),
    ),
    experimental: false,
...
ipfs://QmbRQF3HA9eZ8ZLwyUtnQfXjvZ8MTRnKWTVULhqLALUSTu
```
